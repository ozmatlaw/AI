...
def independent_pair(a1, a2):
    ...
    """
    interfering = len(set(a1.get_pre()).intersection(a2.get_delete())) == 0 and \
                  len(set(a2.get_pre()).intersection(a1.get_delete())) == 0

    inconsistent = len(set(a2.get_add()).intersection(a1.get_delete())) == 0 and \
                   len(set(a1.get_add()).intersection(a2.get_delete())) == 0

    return inconsistent and interfering
...
if __name__ == '__main__':
    ...
    if plan is not None:
        # print("Plan found with %d actions in %.2f seconds" % (len([act for act in plan]), elapsed))
