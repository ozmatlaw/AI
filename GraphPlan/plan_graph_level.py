...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        You should add an action to the layer if its preconditions are in the previous propositions
        ...
        """
        "*** YOUR CODE HERE ***"
        def prerequisite(act, previous_proposition_layer):
            for pre1 in act.get_pre():
                for pre2 in act.get_pre():
                    if pre1 != pre2 and previous_proposition_layer.is_mutex(pre1, pre2):
                        return False
            return previous_proposition_layer.all_preconds_in_layer(act)

        all_actions = PlanGraphLevel.actions

        for action in all_actions:
            if prerequisite(action, previous_proposition_layer):
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        for act1 in current_layer_actions:
            for act2 in current_layer_actions:
                if mutex_actions(act1, act2, previous_layer_mutex_proposition) and act1 != act2:
    ...
    def update_proposition_layer(self):
        ...
        for act in current_layer_actions:
            for prop in act.get_add():
                if prop.get_name() not in prop_dict.keys():
                    new_prop = Proposition(prop.get_name())
                    new_prop.add_producer(act)
                    prop_dict[prop.get_name()] = new_prop

                else:
                    prop_dict[prop.get_name()].add_producer(act)
    ...
    def update_mutex_proposition(self):
        ...
        for prop1 in current_layer_propositions:
            for prop2 in current_layer_propositions:
                if mutex_propositions(prop1, prop2, current_layer_mutex_actions) and prop1 != prop2:
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for prop1 in a1.get_pre():
        for prop2 in a2.get_pre():
            if Pair(prop1, prop2) in mutex_props and prop1 != prop2:
                return True
    return False
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    for prod1 in prop1.get_producers():
        for prod2 in prop2.get_producers():
            if prod1 == prod2 or Pair(prod1, prod2) not in mutex_actions_list:
                return False
    return True
