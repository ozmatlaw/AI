    ...
    def get_successors(self, state):
        ...
        for a in self.actions:
            if a.all_preconds_in_list(state) and not a.is_noop():
                ...
                for elem in a.get_add():
                    succ.add(elem)
                for elem in a.get_delete():
                    if elem in succ:
                        succ.remove(elem)
                successor_list.append((frozenset(succ), a, 1))
...
def max_level(state, planning_problem):
    ...
    layer_num = 0
    graph = dict()
    graph[layer_num] = pg_init
    while not planning_problem.is_goal_state(graph[layer_num].get_proposition_layer().get_propositions()):
        next_pg = PlanGraphLevel()
        next_pg.expand_without_mutex(graph[layer_num])
        layer_num += 1
        graph[layer_num] = next_pg
...
def level_sum(state, planning_problem):
    ...
    graph = dict()
    graph[layer_num] = pg_init
    goals = dict()  # keys=goals, values=level where the key first appeared

    while not planning_problem.is_goal_state(graph[layer_num].get_proposition_layer().get_propositions()):
        for goal in planning_problem.goal:
            if goal in graph[layer_num].get_proposition_layer().get_propositions() and goal not in goals:
                goals[goal] = layer_num
    ...
    return sum(val for val in goals.values())
